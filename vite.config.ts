...
function gracefulShutdownPlugin(): Plugin {
  return {
    name: "app-builder:graceful-shutdown",
    apply: "serve",
    configureServer(server) {
...
/**
 * PGLite loads pglite.data / .wasm next to the bundled server chunk. Nitro
 * traces the JS but not those sidecar files, so local `vite preview` (no
 * DATABASE_URL) crashes unless we copy them into the function output.
 */
...
export default defineConfig(({ command, isPreview }) => {
  const hasCerts = fs.existsSync("server.key") && fs.existsSync("server.crt");
  return {
    server: {
      host: "0.0.0.0",
      port: 8080,
